...
class SalesQAService {
  ...
  /**
   * Clear cache for sales-related queries
   */
  clearSalesCache() {
    console.log('🗑️ Clearing cache for sales-related queries');
    const keysToDelete = [];
    for (const [key, value] of this.cache.entries()) {
      if (key.includes('sale') || key.includes('sales') || key.includes('about')) {
        keysToDelete.push(key);
      }
    }
    keysToDelete.forEach(key => this.cache.delete(key));
    console.log(`🗑️ Cleared ${keysToDelete.length} sales-related cache entries`);
  }
  /**
